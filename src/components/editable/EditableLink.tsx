...
const EditableLink: React.FC<EditableLinkProps> = ({
    ...
    const editableClassName = cn(
        "text-left w-full",
        isLoggedIn
            ? `cursor-pointer border-2 border-red-500 rounded transition-colors underline`
            : className,
    );
    ...
    if (!isLoggedIn) {
        return (
            <a
                ...
                className={cn(
                    "text-rose-300 link hover:link-underline focus-visible:link-underline group-hover:link-underline group-focus-visible:link-under",
                    className,
                )}
            >
                {text}
