...
const ImageCarousel: React.FC<ImageCarouselProps> = ({ limit }) => {
    ...
    const prevImage = () => {
        if (selectedImage !== null) {
            setSelectedImage(
                selectedImage === 0 ? slots.length - 1 : selectedImage - 1,
            );
        }
    };
        ...
        <>
            ...
            <Dialog open={selectedImage !== null} onOpenChange={closeLightbox}>
                <DialogContent
                    ...
                    aria-describedby="Image viewport"
                >
                    {selectedImage !== null && (
                        <div
                            key="imagePreview"
                        ...
                        >
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:bg-white/20 z-10 cursor-pointer"
                                onClick={prevImage}
                            >
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute right-4 top-1/2 -translate-y-1/2 text-white hover:bg-white/20 z-10 cursor-pointer"
                                onClick={nextImage}
                            >
                            ...
                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white bg-black/50 rounded px-3 py-1 text-sm inline-flex space-x-4">
                                <p>
                                    {selectedImage + 1} / {slots.length}
                                </p>
                                <p> - </p>
                                <DialogTitle className="text-sm font-normal">
                                    {weddingData.gallery[selectedImage].caption}
                                </DialogTitle>
                            </div>
                        </div>
                    )}
